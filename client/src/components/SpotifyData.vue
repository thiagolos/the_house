<script setup lang='ts'>
  ...
  // Fetches full artist details from database => sets fullDetails state => sets genreList state => filters out duplicate tracks => sorts by descending popularity => sets trackList state

  apiClient.getOneArtistDetails(props.artistId!)
  .then((data) => fullDetails.value = data)
  .then((data) => trackList.value = data.TopTracks
  .filter((track) => !uniqueNames[track.name] && (uniqueNames[track.name] = true))
  ...
  <div class="spotify-data-container">
    ...
    <div class="social-metrics">
      ...
      <div class="genre-list-container">
        <div class="genre-list" v-if="genreList!.length" v-for="genre of genreList">
          <li>{{ genre.toLowerCase() }}</li>
        </div>
        <div class="genre-list-default" v-else="">
          Artist has no listed genres
        </div>
      </div>
